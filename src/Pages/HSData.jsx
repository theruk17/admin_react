...
const FanData = () => {
  ...
  const init = () => {
    axios
      .post(API_URL + "/admin_data", { t_name: 'headset', c_name: 'hs' })
      .then((res) => {
        setData(res.data);
        setLoading(false)
      });
  ...
  const handleSearch = (value) => {
    const filtered = data.filter((item) =>
      String(item.hs_model).toLowerCase().includes(value.toLowerCase()) ||
      String(item.hs_id).toLowerCase().includes(value.toLowerCase())
    );
