...
const FanData = () => {
  ...
  const handleSearch = (value) => {
    const filtered = data.filter((item) =>
      String(item.f_model).toLowerCase().includes(value.toLowerCase()) ||
      String(item.f_id).toLowerCase().includes(value.toLowerCase())
    );
  ...
  useEffect(() => {
    ...
    const filterData = () => {
      if (selectedBrands !== 'all') {
        Data = Data.filter(item => item.f_brand === selectedBrands);

      }
      if (selectedSubcats !== 'all') {
        Data = Data.filter(item => item.f_group === selectedSubcats);
  ...
  const Column = [
    {
      ...
      width: 60,
      align: 'center',
      render: (text, record) => <a href={record.f_href} target='_blank'><img src={API_URL + '/' + text} alt="" height="30" /></a>,
    },
    {
      title: 'Product name', dataIndex: 'f_model', key: 'f_model',
      render: (_, record) => <><p>{record.f_brand} {record.f_model} {record.f_color}</p>
        <p style={{ lineHeight: 1, fontSize: 10, color: 'Gray' }}><BarcodeOutlined /> {record.f_id}</p></>,
    },
    {
      title: 'Group', dataIndex: 'f_group', key: 'f_group', align: 'center', width: 100,
    },
    {
      title: 'STOCK',
      children: [
  ...
  return (
    <div>
      ...
      <Table loading={loading} dataSource={filteredData} columns={Column} rowKey={record => record.f_id} pagination={pagination} onChange={onChange} bordered size="small"></Table>
      <EditForm
        visible={visible}
