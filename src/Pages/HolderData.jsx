...
const EditForm = ({ visible, onCreate, onCancel, record }) => {
  ...
  useEffect(() => {
    setFileList([{ url: API_URL + '/' + record.holder_img }])
    form.setFieldsValue({
      brand: record.holder_brand,
      model: record.holder_model,
      price_srp: record.product_price,
      discount: record.product_minprice,
      href: record.holder_href,
  ...
  const handleUpload = ({ file, onSuccess, onError, onProgress }) => {
    ...
    formData.append('id', record.holder_id);
    formData.append('t_name', 'holder');
    formData.append('c_name', 'holder');
...
const FanData = () => {
  ...
  const handleSearch = (value) => {
    const filtered = data.filter((item) =>
      String(item.holder_model).toLowerCase().includes(value.trim().toLowerCase()) ||
      String(item.holder_id).toLowerCase().includes(value.trim().toLowerCase())
    );
  ...
  useEffect(() => {
    ...
    const filterData = () => {
      if (selectedBrands !== 'all') {
        Data = Data.filter(item => item.holder_brand === selectedBrands);
  ...
  const handleDelete = id => {
    axios
      .delete(API_URL + `/admin_del_holder/${id}`)
      .then(res => {
  ...
  const handleCreate = (values) => {
    axios.put(API_URL + '/edit_holder/' + record.sku, values)
      .then(res => {
        setVisible(false)
        message.success(res.data);
        init()
  ...
  const Column = [
    {
      title: 'Image',
      dataIndex: 'holder_img',
      key: 'holder_img',
      width: 60,
      align: 'center',
      render: (text, record) => <a href={record.holder_href} target='_blank'><img src={API_URL + '/' + text} alt="" height="30" /></a>,
    },
    {
      title: 'Product name', dataIndex: 'holder_model', key: 'holder_model',
      render: (_, record) => <><p>{record.holder_brand} {record.holder_model} {record.holder_color}</p>
        <p style={{ lineHeight: 1, fontSize: 10, color: 'Gray' }}><BarcodeOutlined /> {record.holder_id}</p></>,
    },
    {
      title: 'STOCK',
      children: [
        ...
        {
          title: 'ข่อนแก่น', dataIndex: 'stock_khonkean', key: 'stock_khonkean', align: 'center', width: 70,
  ...
  return (
    <div>
      ...
      <Table loading={loading} dataSource={filteredData} columns={Column} rowKey={record => record.holder_id} pagination={pagination} onChange={onChange} bordered size="small"></Table>
      <EditForm
        visible={visible}
